/*
    This program will modify a previously created singly linked list that accepts size values from 20 to 100. The size value will be used to randomly
    generate an amount of data equal to size that will be used to create a doubly linked list that will be displayed. Then, the even number
    values of the linked list will be deleted such that only odd values remain. After this new list is displayed, the values will then be printed in
    reverse order using the previous pointer of the linked list.

    April 18, 2020

    Jackie Diep
    University of Southern Mississippi
    307 Data Structures Homework Assignment 5
*/
        ...
        else
        {
            ...
            for (int i = 0; i < size; i++)
            {
                ...
                else
                {
                    ...
                    //Set newNode's previous pointer to point to the current node
            ...
            //Call the function to print the odd only list
            cout << "Odd Only list" << endl;
            printList(head);

            //Call the function to print the odd only list backwards
            cout << "Odd Only list backwards" << endl;
...
//Function definition to delete the even numbered values of the linked list
void deleteEven(node* head)
{
    ...
    //While loop to traverse the list until the end when temp = NULL
    while (temp != NULL)
    {

        //If temp's number value is even
        if (temp->number % 2 == 0)
        {

            //If temp is the head, do not change the previous node
            if(temp->previous != NULL)
                temp->previous->next = temp->next;

            //If temp is the end, do not change the next node
            if (temp->next != NULL)
                temp->next->previous = temp->previous;

            //Move temp forward in the while loop
            temp = temp->next;
        }

        //If temp's number value is odd
        else
        {
            //Move temp forward in the while loop
            temp = temp->next;
...
void printListBackwards(node* head)
{
    ...
    //Traverse the list until the end when temp's next pointer = NULL
    while (temp->next != NULL)
    {
        temp = temp->next;
    }

    //Until temp's previous is NULL, which signifies the head of the list, output elements of the linked list separated by tabs
    while (temp->previous != NULL)
    {
        ...
        //Move temp backwards and increment the counter
        temp = temp->previous;
        counter++;
    }
    cout << endl << endl;
